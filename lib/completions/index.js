...
const {structs, getStructElems, isStruct} = require('./structs')
...
function find (searchFor, docText) {
  ...
  const structArray = structs(docText);
  if (searchFor.indexOf('.') > -1) {
    searchFor = searchFor.slice(0,searchFor.length-1);
  ...
  let variables = allVariables(docText);
  variables = variables.concat(findFromDelimeter('.',requireContext).map(item => new Item(item.insertText, item.kind)));

  return fuzz.filter(index.concat(variables).concat(structArray), searchFor, { key: 'label' });
}

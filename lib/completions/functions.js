...
const variabl = require('./variables');
let allMethodItems = [];
...
/**
 * parse document 
 * add parametres to autocomplete variables
 * 
 * @param {string} docText 
 * @return {Null}
 */
function parseFunctions(docText) {
    ...
    try {
        ...
        for (var funcName in signatures) {
            ...
            let index = 0;
            if (signatures[funcName]['params'].length > 0) {
                for (var params of signatures[funcName]['params']) {
                    if (params == null || params == undefined) continue;
                    variabl.parse("declare " + params);   // add function parameters to autocomplete variables
...
/**
 * get all function autocomplete items
 * @return {Item[]} 
 */
 ...
 */
function getFunctionReturnType(funcName) {
    if (allMethods.hasOwnProperty(funcName)) {
        return allMethods[funcName]['returnType'];
    }
    return null;
}
